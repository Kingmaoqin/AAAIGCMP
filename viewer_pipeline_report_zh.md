...
### 8.5 新增：论文风格协议 benchmark

为了和新增论文中的机器学习实验协议做更接近的对照，本项目额外增加了一条独立 benchmark 线路，而不是覆盖原先的 grouped CV 主结果。

独立脚本为：

- `newtry_zif8_modeling/run_paper_style_benchmark.py`

输出目录为：

- `newtry_zif8_modeling/paper_style_benchmark/`
...
- 随机 `80/20` 划分训练集与测试集
- 对目标变量 `TEM_SEM_Diameter` 做右尾 winsorization
- 右尾 winsorization 分位数设为 `0.975`
- 当前实际 winsorization 阈值为 `1871.25 nm`
- 仅对训练集做 `5%` 局部插值增强
- 增强方法是 `SMOTER-inspired` 的近邻局部插值实现
- 在增强后的训练集上做 `5-fold CV` 调参与模型选择
- 最后在固定测试集上报告测试指标

协议摘要保存在：

- `paper_style_benchmark/logs/paper_style_protocol.json`

#### 8.5.2 benchmark 结果表

相关结果表包括：

- `paper_style_benchmark/tables/paper_style_model_ranking.csv`
- `paper_style_benchmark/tables/paper_style_results_all.csv`
- `paper_style_benchmark/tables/paper_style_cv_trials.csv`
- `paper_style_benchmark/tables/paper_style_test_predictions.csv`

#### 8.5.3 论文风格协议下的最佳模型

在这条 benchmark 线路下，当前最佳模型变为：
...
- `HistGradientBoostingRegressor (log)`
- `AdaBoostRegressor (log)`
- `GradientBoostingRegressor (log)`
- `ExtraTreesRegressor (log)`

#### 8.5.4 与 grouped nested CV 主结果的关系

这一点必须明确：

- `grouped nested CV` 的主结果仍然是本项目默认、也是更严格的主评估结果。
- `paper-style benchmark` 是为了贴近论文协议而新增的补充实验。
- 两套结果不能简单混为一谈。

当前两条主线的最优模型分别是：

- 严格 `grouped nested CV`：`BaggingRegressor (log)`
- 论文风格 benchmark：`LightGBMRegressor (log)`

这说明：

- 在更宽松的随机划分与训练集增强协议下，boosting 类模型明显更占优。
- 在跨 DOI 的严格泛化设定下，模型排名会发生变化。
...
并支持在请求中指定当前模型键，以便预设样本在不同模型下都能即时查看预测差异。
...
- 在两条独立实验线的最佳模型之间切换
- 不直接替代原有 grouped CV 最佳模型
- 用同一组输入参数，在不同模型下实时得到不同预测结果

当前可切换的模型包括：

- `Grouped CV Best`
  - 当前对应 `BaggingRegressor (log)`
  - 协议为 `nested grouped CV with DOI grouping`
- `Paper-style Best`
  - 当前对应 `LightGBMRegressor (log)`
  - 协议为 `random 80/20 split + winsorization + 5% train augmentation + 5-fold CV tuning`

这样做的目的是：

- 保留严格评估模型作为默认主模型
- 同时允许用户查看“论文风格协议下更高分模型”的预测表现
- 避免直接覆盖、混淆两套不同验证逻辑下的结果
